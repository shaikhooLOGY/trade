...
if ($_SERVER['REQUEST_METHOD']==='POST') {
  ...
  // Allow bypass for E2E testing in local environment
  $csrfToken = $_POST['csrf'] ?? '';
  if (!validate_csrf($csrfToken) && !getenv('ALLOW_CSRF_BYPASS')) {
    ...
    if ($name===''||$email===''||$pass===''||$conf==='') {
      ...
      if ($st=$mysqli->prepare("SELECT id, status, email_verified FROM users WHERE email=? LIMIT 1")){
        ...
        if ($ex) {
          ...
          // Create minimal user as 'pending'
          ...
          if (!$ins){ $err = 'DB error: '.$mysqli->error; }
            ...
            $ins->bind_param('ssssss',$name,$email,$pass,$hash,$status,$created_at);
            if ($ins->execute()){
              $uid = $ins->insert_id;
              $ins->close();
