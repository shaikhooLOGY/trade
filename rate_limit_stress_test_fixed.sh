...
test_endpoint() {
    ...
    for i in {1..12}; do
        ...
        # Execute curl request with simpler output parsing
        temp_response=$(mktemp)
        temp_headers=$(mktemp)
        
        # Execute curl with better output control
        ...
        status_code=$(grep "HTTP_STATUS:" "$temp_response" | cut -d':' -f2)
        time_total=$(grep "TIME_TOTAL:" "$temp_response" | cut -d':' -f2)
        
        # Remove the curl status lines from response
        sed -i '' '/HTTP_STATUS:/d' "$temp_response"
        sed -i '' '/TIME_TOTAL:/d' "$temp_response"
        
        # Check for rate limiting headers
        retry_after=$(grep -i "retry-after:" "$temp_headers" | cut -d' ' -f2- | tr -d '\r\n')
        rate_limit_limit=$(grep -i "x-ratelimit-limit:" "$temp_headers" | cut -d' ' -f2- | tr -d '\r\n')
        rate_limit_remaining=$(grep -i "x-ratelimit-remaining:" "$temp_headers" | cut -d' ' -f2- | tr -d '\r\n')
        rate_limit_reset=$(grep -i "x-ratelimit-reset:" "$temp_headers" | cut -d' ' -f2- | tr -d '\r\n')
        ...
        # Get a preview of response body
        body_preview=$(head -c 200 "$temp_response")
        echo "  Body Preview: ${body_preview:0:200}..." >> "$RESULTS_FILE"
        echo "" >> "$RESULTS_FILE"
